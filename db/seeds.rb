...
Category.find_by(title: 'HTML').tests << Test.create(title: 'Основы HTML')
Category.find_by(title: 'Ruby').tests << Test.create(title: 'Основы Ruby')
Category.find_by(title: 'Java').tests << Test.create(title: 'Основы Java')
Category.find_by(title: 'JavaScript').tests << Test.create(title: 'Основы JavaScript')
Category.find_by(title: 'Викторины').tests.push(Test
.create(title: 'Викторина: Планеты солнечной системы'))
Category.find_by(title: 'Викторины').tests << Test.create(title: 'Викторина: Да/нет')
Category.find_by(title: 'Викторины').tests << Test.create(title: 'Викторина: Животные')

# Basic HTML: questions/answers
html_basic_test = Test.find_by(title: 'Основы HTML')
html_basic_test.questions << Question.create(
  body: 'О чем говорит тэг <p align="right"> … </p>?'
)
html_basic_test.questions.last.answers << Answer.create(
...
ruby_basic_test.questions << Question.create(
  body: 'Где записана сокращенная форма кода ниже:
x = x / 3'
)
ruby_basic_test.questions.last.answers << Answer.create(
  body: 'x /= 3 ',
  correct: true
)
ruby_basic_test.questions.last.answers << Answer.create(
...
java_basic_test.questions << Question.create(
...
java_basic_test.questions.last.answers << Answer.create(
  body: '52'
)
java_basic_test.questions.last.answers << Answer.create(
  body: '7',
...
java_script_basic_test.questions << Question.create(
  body: 'Какую функцию нужно использовать, чтобы пользователь мог ввести свой текст
  (ответ) в окошке?'.gsub("\n ", '')
)
java_script_basic_test.questions.last.answers << Answer.create(
  body: 'print'
)
java_script_basic_test.questions.last.answers << Answer.create(
  body: 'prompt',
...
quiz_planets_test.questions << Question.create(
  body: 'Сколько спутников у Марса?',
  answer_explanation: 'У Марса два спутника: Фобос и Деймос.'
)
quiz_planets_test.questions.last.answers << Answer.create(
  body: '13'
)
quiz_planets_test.questions.last.answers << Answer.create(
  body: '2',
...
quiz_yes_no_test.questions << Question.create(
...
quiz_yes_no_test.questions.last.answers << Answer.create(
  body: 'Да'
)
quiz_yes_no_test.questions.last.answers << Answer.create(
  body: 'Нет',
...
quiz_animals_test.questions << Question.create(
...
quiz_animals_test.questions.last.answers << Answer.create(
  body: 'Лев'
)
quiz_animals_test.questions.last.answers << Answer.create(
  body: 'Тигр',
...
Category.all.count.times do |db_category_id|
  3.times do |i|
    rand(1..3).times do |j|
      test_level = i + 1
      Category.find_by(id: db_category_id + 1).tests << Test.create(
...
Test.where("title LIKE 'Болванка%'").each do |db_test|
  rand(5..15).times do
    db_test.questions << Question.create(
    ...
    question_answer_count.times do |i|
      db_test.questions.last.answers << Answer.create(
...
User.all.each do |user|
  ...
  Test.count.times do |i|
    ...
    user.tests << Test.find_by(id: i + 1)
...
#   User.all.each do |user|
#     puts "#{'=' * 10}\nid: #{user.id}\nname: #{user.name}"
#     user.tests.length.zero? ? (puts '-no started tests-') : (puts "tests: #{user.tests.inspect}")
